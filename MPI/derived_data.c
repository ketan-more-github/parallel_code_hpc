...
int main(int argc, char* argv[])
{
    ...
    MPI_Get_address(&dummy_person.id, &displacements[2]);
    MPI_Get_address(&dummy_person.name[0], &displacements[3]);
    ...
    MPI_Type_create_struct(4, lengths, displacements, types, &person_type);
    ...
    switch(my_rank)
    {
        case SENDER:
        {
            ...
            buffer.name[9] = '\0';
            printf("MPI process %d sends person:\n\t- age = %d\n\t- id = %d\n\t- height = %f\n\t- name = %s\n", my_rank, buffer.age,buffer.id , buffer.height, buffer.name);
        ...
        case RECEIVER:
        {
            ...
            printf("MPI process %d sends person:\n\t- age = %d\n\t- id = %d\n\t- height = %f\n\t- name = %s\n", my_rank, received.age,received.id , received.height, received.name);
